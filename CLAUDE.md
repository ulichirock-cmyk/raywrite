...
- 发布：改 `package.json` version → `git tag vX.Y.Z && git push origin vX.Y.Z`，Actions（`.github/workflows/release.yml`）在 Windows runner 构建并把 exe 挂上 GitHub Release。
...
- `web/src/settingsStore.js` + `components/SettingsPanel.vue`：界面设置（卡片字体族/字号、长卡折叠开关），localStorage key `agentText.settings`。字体经 App.vue 注入 CSS 变量 `--card-font`/`--card-font-size` 由 `.editor-body` 消费；长卡折叠由 Card.vue 的 ResizeObserver 按内容高度 >260px 判定，出「展开/收起」钮。Electron 窗口菜单栏已用 `Menu.setApplicationMenu(null)` 去除。
- `pathHighlight.js` 的 `PATH_RE` 同时认 `C:\...`、`C:/...` 和 Unix 路径。

### Electron 壳（electron/main.mjs，ESM 主进程）

`app.isPackaged` 决定数据根默认值（`~/agentText`）；启动前先探测 7777 端口有无现成 server，有则复用、没有才 `import('../server/index.mjs')`（import 即 listen）。关闭按钮隐藏到托盘保活，托盘「退出」走统一的 `quitApp()`：先 `win.close()` 给 pagehide 的 sendBeacon 兜底保存留时间再 `app.quit()`。**自更新**（`electron/updater.mjs`）：portable 目标没有 electron-updater 支持，自己实现「自举替换」——查 GitHub latest release（REPO 常量硬编码在文件顶部）比版本 → 弹窗确认 → 下载到 `PORTABLE_EXECUTABLE_FILE` 旁（`.part`，按 asset.size 校验）→ 转正为新版 exe 并写协议文件 `agentText-update.json`（`{phase:'replace', src, dst}`）→ 退出时用 **explorer 代理**启动新 exe（父进程是 shell）→ 新版进程在 `runUpdateHandoff()`（main.mjs 拿单实例锁之前调）里等旧 exe 解锁 → 删旧 → 把自身复制回旧路径（保快捷方式）→ 协议转 `cleanup` → explorer 起旧路径新版自退，接力实例后台删临时 exe；全程写 `agentText-update.log`。**为什么绕这么大圈**（都实测踩过）：app 直接 spawn 的子进程（含 detached）会随进程树被连坐杀掉；PowerShell/WMI/cmd 工具链会被安全软件选择性拦截。所以替换只用自身 exe + explorer。**每个版本必须保留 runUpdateHandoff 协议处理**，否则升级链断裂。打包形态启动 5s 后静默检查 + 每 6h 一次，托盘菜单可手动「检查更新」；非 portable 形态退化为打开 Release 下载页。单实例锁二次启动聚焦已有窗口。electron-builder 配置在 package.json `build` 字段：win portable 目标，files 白名单只含 `electron/`、`server/`、`web/dist/`、`package.json`——**绝不打包 `assets/`、`data/`（用户数据）**。图标（exe/窗口/托盘/favicon 同款设计）由 `node scripts/gen-icon.mjs` 再生（零依赖手写 PNG/ICO 编码），产出进 git 的 `electron/icon.ico|icon.png|tray.png`。
